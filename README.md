...
yominterp (ヨミンタープ) は、英語で書かれた Z-machine のインタラクティブフィクション (ZORK のようなテキストアドベンチャー) を、AI 翻訳をはさんで**日本語のまま遊べる**ようにするプレイヤーです。(Glulx 対応は今後の予定です)
...
> - **macOS** 版は署名・公証 (notarization) 済みです。ダウンロードしてそのまま開けます。
> - **Windows** 版は署名されていないため、初回起動時に SmartScreen の警告が出ます。
>   **「詳細情報」→「実行」**を選んでください。
